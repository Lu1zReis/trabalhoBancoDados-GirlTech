<?php 
 ...
 class ClienteDao { 
     ...
     public function Create(Cliente $p) { 
         ...
         $sql = 'INSERT INTO cliente (cpf,endereço,nome,telefone1,cod_loja, cod_atendente) VALUES (?,?,?,?,?,?,?)'; 
         ...
         $stmt->bindValue(3, $p->getNome()); 
         $stmt->bindValue(4, $p->getTelefone()); 
         $stmt->bindValue(5, $p->getCod_loja()); 
         $stmt->bindValue(6, $p->getCod_atendente()); 
     ...
     public function Update(Cliente $p) { 
         ...
         $sql = 'UPDATE cliente SET cpf = ?, endereço = ?, nome = ?, telefone1 = ?, cod_loja = ?, cod_atendente WHERE cod = ?'; 
  
         $stmt = Conn::getConn()->prepare($sql); 
  
         // aonde vai retornar os valores e atualizá-los
         $stmt->bindValue(1, $p->cpf()); 
         $stmt->bindValue(2, $p->getEndereco()); 
         $stmt->bindValue(3, $p->getNome()); 
         $stmt->bindValue(4, $p->getTelefone1()); 
         $stmt->bindValue(5, $p->getCod_loja());
         $stmt->bindValue(6, $p->getCod_atendente()); 
