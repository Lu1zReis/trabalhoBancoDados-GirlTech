<?php 
 ...
 class FornecedorDao { 
     ...
     public function Create(Fornecedor $p) { 
         ...
         $sql = 'INSERT INTO fornecedor (cod_loja, cep, bairro, rua, numero, cnpj, telefone) VALUES (?,?,?,?,?,?,?)'; 
         ...
         $stmt->bindValue(1, $p->getCod_loja()); 
         $stmt->bindValue(2, $p->getCep()); 
         $stmt->bindValue(3, $p->getBairro()); 
         $stmt->bindValue(4, $p->getRua()); 
         $stmt->bindValue(5, $p->getNumero()); 
         $stmt->bindValue(6, $p->getCnpj()); 
     ...
     public function Update(Fornecedor $p) { 
         ...
         $sql = 'UPDATE fornecedor SET cod_loja=?, cep=?, bairro=?, rua=?, numero=?, cnpj=?, telefone=? WHERE cod = ?'; 
