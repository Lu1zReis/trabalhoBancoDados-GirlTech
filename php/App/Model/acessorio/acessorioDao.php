<?php 
 ...
 class AcessorioDao { 
     ...
     public function Update(Acessorio $p) { 
      
         $sql = 'UPDATE acessorio SET nome = ?, cod_produto = ? WHERE nome = ?'; 
     ...
     public function Delete($n) { 
         ...
         $sql = 'DELETE FROM acessorio WHERE nome = ?'; 
         $stmt = Conn::getConn()->prepare($sql); 
         $stmt->bindValue(1, $n); 
         $stmt->execute(); 
