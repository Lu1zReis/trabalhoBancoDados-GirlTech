<?php 
 ...
 class AssistenteDao { 
     ...
     public function Update(Assistente $p) { 
      
         $sql = 'UPDATE assistente SET cod_func = ? WHERE cod = ?'; 
  
         $stmt = Conn::getConn()->prepare($sql); 
  
         // aonde vai retornar os valores e atualizá-los 
         $stmt->bindValue(1, $p->getCod_func()); 
         $stmt->bindValue(2, $p->getCod()); 
     ...
     public function Delete($c) { 
         ...
         $sql = 'DELETE FROM assistente WHERE cod = ?'; 
         $stmt = Conn::getConn()->prepare($sql); 
         $stmt->bindValue(1, $c); 
         $stmt->execute(); 
