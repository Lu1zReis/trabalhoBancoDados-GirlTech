<?php 
 ...
 class ItemDao { 
     ...
     public function Create(Item $p) { 
         ...
         $stmt->bindValue(1, $p->getCod_ordem()); 
         $stmt->bindValue(2, $p->getCod_aparelho()); 
     ...
     public function Update(item $p) { 
      
         $sql = 'UPDATE item SET cod_ordem=?, cod_aparelho=? WHERE cod_ordem = ?'; 
         ...
         $stmt->bindValue(1, $p->getCod_ordem()); 
         $stmt->bindValue(2, $p->getCod_aparelho()); 
         $stmt->bindValue(3, $p->getCod_ordem()); 
     ...
     public function Delete($c) { 
         ...
         $sql = 'DELETE FROM item WHERE cod_ordem = ?'; 
         $stmt = Conn::getConn()->prepare($sql); 
         $stmt->bindValue(1, $c); 
         $stmt->execute(); 
