<?php 
 ...
 class EntregadorDao { 
     ...
     public function Update(Entregador $p) { 
      
         $sql = 'UPDATE entregador SET cod_func = ? WHERE cod = ?'; 
     ...
     public function Delete($c) { 
         ...
         $sql = 'DELETE FROM entregador WHERE cod = ?'; 
         $stmt = Conn::getConn()->prepare($sql); 
         $stmt->bindValue(1, $c); 
         $stmt->execute(); 
