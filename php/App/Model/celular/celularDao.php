<?php 
 ...
 class celularDao { 
     ...
     public function Create(Celular $p) { 
         ...
         $stmt->bindValue(1, $p->getSerie()); 
         $stmt->bindValue(2, $p->getCod_produto()); 
         $stmt->bindValue(3, $p->getMarca()); 
         $stmt->bindValue(4, $p->getSistema()); 
     ...
     public function Update(Celular $p) { 
      
         $sql = 'UPDATE celular SET serie = ?, cod_produto = ?, marca = ?, sistema = ? WHERE serie = ?'; 
     ...
     public function Delete($c) { 
         ...
         $sql = 'DELETE FROM celular WHERE serie = ?'; 
         $stmt = Conn::getConn()->prepare($sql); 
         $stmt->bindValue(1, $c); 
         $stmt->execute(); 
