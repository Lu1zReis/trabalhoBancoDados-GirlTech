<?php 
 ...
 class AtendenteDao { 
     ...
     public function Update(Atendente $p) { 
      
         $sql = 'UPDATE atendente SET cod_func = ? WHERE cod = ?'; 
     ...
     public function Delete($c) { 
         ...
         $sql = 'DELETE FROM atendente WHERE cod = ?'; 
         $stmt = Conn::getConn()->prepare($sql); 
         $stmt->bindValue(1, $c); 
         $stmt->execute(); 
