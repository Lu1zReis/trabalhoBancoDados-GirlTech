<?php 
 ...
 class VeiculoDao { 
     ...
     public function Create(Veiculo $p) { 
         ...
         $stmt->bindValue(1, $p->getPlaca()); 
         $stmt->bindValue(2, $p->getRenavam()); 
         $stmt->bindValue(3, $p->getModelo()); 
         $stmt->bindValue(4, $p->getTipo()); 
         $stmt->bindValue(5, $p->getCod_entregador()); 
     ...
     public function Update(Veiculo $p) { 
      
         $sql = 'UPDATE veiculo SET placa=?, renavam=?, modelo=?, tipo=?, cod_entregador=? WHERE placa = ?'; 
