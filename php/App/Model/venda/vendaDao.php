<?php
 ...
 class VeiculoDao {
     ...
     public function Create(Veiculo $p) {
         ...
         $sql = 'INSERT INTO venda (cod_cliente, cod_atendente, cod_produto, data) VALUES (?,?,?,?)';
         ...
         $stmt->bindValue(1, $p->getCod_cliente());
         $stmt->bindValue(2, $p->getCod_atendente());
         $stmt->bindValue(3, $p->getProduto());
     ...
     public function Update(Veiculo $p) {
         ...
         $sql = 'UPDATE veiculo SET cod_cliente=?, cod_atendente=?, cod_produto=?, data=? WHERE cod = ?';
     ...
     public function Delete($c) {
         ...
         $sql = 'DELETE FROM venda WHERE cod = ?';
         $stmt = Conn::getConn()->prepare($sql);
         $stmt->bindValue(1, $c);
         $stmt->execute();
