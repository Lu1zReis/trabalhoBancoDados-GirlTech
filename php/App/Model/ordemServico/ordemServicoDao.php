<?php 
 ...
 class OrdemServicoDao { 
     ...
     public function Create(OrdemServico $p) { 
  
         // podemos passar os valores no VALUES como (?,?) e depois colocar em sequência a numeração para inserir no banco de dados  
         $sql = 'INSERT INTO ordem_servico (descricao, valor, data_abertura, data_finalizacao, cod_func) VALUES (?,?,?,?,?)'; 
         ...
         $stmt->bindValue(3, $p->getData_abertura()); 
         $stmt->bindValue(4, $p->getData_finalizacao()); 
         $stmt->bindValue(5, $p->getCod_func()); 
     ...
     public function Update(OrdemServico $p) { 
      
         $sql = 'UPDATE ordem_servico SET descricao=?, valor=?, data_abertura=?, data_finalizacao=?, cod_func=? WHERE cod = ?'; 
     ...
     public function Delete($c) { 
         ...
         $sql = 'DELETE FROM ordem_servico WHERE cod = ?'; 
         $stmt = Conn::getConn()->prepare($sql); 
         $stmt->bindValue(1, $c); 
         $stmt->execute(); 
